              ...
              <div className="flex flex-wrap gap-2">
                {formData.times.map((time, index) => (
                  <button
                    ...
                    className={`flex items-center rounded-full px-3 py-1 text-sm transition-colors ${formData.selectedTimes.includes(time)
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
      ...
      <div className="flex items-center justify-between mt-auto text-xs">
        ...
        <div
          className={`px-2 py-1 rounded-full font-medium ${reminder.active
...
const Reminders = () => {
  ...
  // Mock initial data
  const mockInitialMedications = [
    { id: 1, name: "Lisinopril", dosage: "10mg", time: ["08:00"], frequency: "Daily" },
    { id: 2, name: "Metformin", dosage: "500mg", time: ["08:00", "20:00"], frequency: "Twice Daily" },
  ];

  const mockInitialReminders = [
    { id: 1, medication_id: 1, type: "whatsapp", reminder_time: new Date().setHours(8, 0, 0, 0), frequency: "Daily", status: "active", user_id: session?.user?.id },
    { id: 2, medication_id: 2, type: "email", reminder_time: new Date().setHours(20, 0, 0, 0), frequency: "Twice Daily", status: "active", user_id: session?.user?.id },
  ];

  const [reminders, setReminders] = useState(mockInitialReminders)
  const [medications, setMedications] = useState(mockInitialMedications)
  const [combinedReminders, setCombinedReminders] = useState([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  const fetchData = async () => {
    // Simulated fetching for the mock data
    setLoading(true)
    setTimeout(() => {
      setReminders(mockInitialReminders)
      setMedications(mockInitialMedications)
      setLoading(false)
    }, 500)
  }
  ...
  const handleSave = async (formData) => {
    try {
      const newReminders = formData.times.map(time => {
        const [hours, minutes] = time.split(":");
        const reminderDate = new Date();
        reminderDate.setHours(hours, minutes, 0, 0);

        return {
          id: Date.now() + Math.random(), // Mocking ID generator
          medication_id: parseInt(formData.medication_id), // Ensure it's treated as number for mock lookup
      ...
      if (editingReminder) {
        // Remove the edited reminder from the old array, add the newly generated ones
        const filteredReminders = reminders.filter(r => r.id !== editingReminder.id)
        setReminders([...newReminders, ...filteredReminders]);
      } else {
